...
import type { IUser, LoginUserInput, LoginUserResponse, UserInformation } from '../../interfaces/user'
...
export const fetchLogin = createAsyncThunk('user/fetchLogin', async ({ citizenId, laserId }: LoginUserInput) => {
...
export const userSlice = createSlice({
  ...
  reducers: {
    ...
    setAuthUser: (state: IUser, action: PayloadAction<UserInformation | null>) => {
  ...
  extraReducers: (builder) => {
    builder.addCase(fetchLogin.fulfilled, (state, action: PayloadAction<LoginUserResponse>) => {
...
export const { setIsAuthenticated, setIsAcceptedRules, setAuthUser } = userSlice.actions
